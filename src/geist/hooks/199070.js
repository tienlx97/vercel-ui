...
import { getActiveElement, getEventTarget, nodeContains } from '@/geist/utils/755167'
...
export function useFocusWithin(options) {
  const { isDisabled, onBlurWithin, onFocusWithin, onFocusWithinChange } = options
  ...
  const triggerSyntheticBlurHandling = useSyntheticBlurEvent(handleBlurWithin)
        ...
        addGlobalListener(
          ...
          (nativeFocusEvent) => {
            if (
              ...
              const syntheticBlur = new ownerDocument.defaultView.FocusEvent('blur', {
