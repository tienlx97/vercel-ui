'use client'

import clsx from 'clsx'
import React from 'react'
import styles from '../css/book.module.css'
import { Text } from './Text'
import { mapResponsiveProp } from '../utils/783151'
import Stack from './Stack'
import { Icon } from './icon/Icon'

function DefaultBookIllustration() {
  return (
    <svg fill="none" height="56" viewBox="0 0 36 56" width="36" xmlns="http://www.w3.org/2000/svg">
      <path
        clipRule="evenodd"
        d="M3.03113 28.0005C6.26017 23.1765 11.7592 20.0005 18 20.0005C24.2409 20.0005 29.7399 23.1765 32.9689 28.0005C29.7399 32.8244 24.2409 36.0005 18 36.0005C11.7592 36.0005 6.26017 32.8244 3.03113 28.0005Z"
        fill="#0070F3"
        fillRule="evenodd"
      />
      <path
        clipRule="evenodd"
        d="M32.9691 28.0012C34.8835 25.1411 36 21.7017 36 18.0015C36 8.06034 27.9411 0.00146484 18 0.00146484C8.05887 0.00146484 0 8.06034 0 18.0015C0 21.7017 1.11648 25.1411 3.03094 28.0012C6.25996 23.1771 11.7591 20.001 18 20.001C24.2409 20.001 29.74 23.1771 32.9691 28.0012Z"
        fill="#45DEC4"
        fillRule="evenodd"
      />
      <path
        clipRule="evenodd"
        d="M32.9692 28.0005C29.7402 32.8247 24.241 36.001 18 36.001C11.759 36.001 6.25977 32.8247 3.03077 28.0005C1.11642 30.8606 0 34.2999 0 38C0 47.9411 8.05887 56 18 56C27.9411 56 36 47.9411 36 38C36 34.2999 34.8836 30.8606 32.9692 28.0005Z"
        fill="#E5484D"
        fillRule="evenodd"
      />
    </svg>
  )
}

//
function getStableTextureRotationKey(value) {
  let hash = 0

  for (let index = 0; index < value.length; index += 1) {
    hash = (hash << 5) - hash + value.codePointAt(index)
    hash = Math.trunc(hash)
  }

  return hash % 2 === 0
}

function Book({
  title,
  icon = 'logo-vercel',
  className,
  variant = 'stripe',
  color = variant === 'stripe' ? 'var(--ds-amber-600)' : undefined,
  width = 196,
  illustration,
  textColor,
  textured = false,
}) {
  //
  // const { shouldRotate } = React.useMemo(() => {
  //   if (!textured) {
  //     return { textureXOffset: '0%', shouldRotate: false }
  //   }

  //   // eslint-disable-next-line react-hooks/purity
  //   return { shouldRotate: Math.random() > 0.5 }
  // }, [textured])

  const { shouldRotate } = React.useMemo(() => {
    if (!textured) {
      return { shouldRotate: false }
    }

    return {
      shouldRotate: getStableTextureRotationKey(`${title}-${color}-${variant}`),
    }
  }, [color, textured, title, variant])

  return (
    <div
      className={clsx(styles.perspective, className)}
      style={mapResponsiveProp('book-width', width)}
    >
      <div
        className={clsx(styles['rotate-wrapper'], styles[variant], color && styles.color)}
        style={{
          '--book-color': color,
          '--book-text-color': textColor,
        }}
      >
        <Stack className={styles.book} direction="column">
          {variant === 'stripe' ? (
            <Stack aria-hidden className={styles.stripe} direction="row" gap={2}>
              <div className={styles.illustration}>{illustration}</div>
              <div className={styles.bind} />
            </Stack>
          ) : null}

          <Stack className={styles.body} direction="row">
            <div aria-hidden className={styles.bind} />

            <Stack
              className={styles.content}
              justify={variant === 'stripe' ? 'space-between' : 'flex-start'}
            >
              <Text as="span" className={styles.title} weight={600}>
                {title}
              </Text>

              {variant === 'stripe' ? (
                <Icon name={icon} />
              ) : (
                <div className={styles.illustration}>
                  {illustration || <DefaultBookIllustration />}
                </div>
              )}
            </Stack>
          </Stack>

          {textured ? (
            <div
              aria-hidden
              className={styles.texture}
              style={{
                transform: shouldRotate ? 'rotate(180deg)' : 'rotate(0deg)',
              }}
            />
          ) : null}
        </Stack>

        <div aria-hidden className={clsx(styles.pages, textured && styles.textured)} />
        <div aria-hidden className={styles.back} />
      </div>
    </div>
  )
}

export default Book
